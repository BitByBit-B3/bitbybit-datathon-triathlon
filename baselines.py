...
class MedianBaseline:
    ...
    def __init__(self, fallback_levels: list[list[str]], target_col: str = "target") -> None:
        """
    ...
    def fit(self, features_df: pd.DataFrame, target_series: pd.Series) -> "MedianBaseline":
        """
        ...
        for level_idx, feature_cols in enumerate(self.fallback_levels):
            ...
            if isinstance(grouped.index, pd.MultiIndex):
                median_dict.update(grouped.to_dict())
            else:
    ...
    def predict(self, features_df: pd.DataFrame) -> np.ndarray:
        ...
        for _idx, row in features_df.iterrows():
        ...
        logger.info(
            f"Prediction stats: min={predictions.min():.2f}, max={predictions.max():.2f}, mean={predictions.mean():.2f}"
        )
    ...
    def _predict_single(self, row: pd.Series, level_usage_count: dict) -> float:
        ...
        for level_idx, feature_cols in enumerate(self.fallback_levels):
            ...
            lookup_key = (key_values[0],) if len(key_values) == 1 else tuple(key_values)
...
class SimpleStatsBaseline:
    ...
    def __init__(self, key_cols: list[str], stat_type: str = "median") -> None:
        """
    ...
    def fit(self, features_df: pd.DataFrame, target_series: pd.Series) -> "SimpleStatsBaseline":
