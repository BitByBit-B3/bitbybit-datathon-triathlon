...
def validate_predictions(task_name, predictions_path, test_inputs_path):
    ...
    if list(pred_df.columns) != expected_cols:
        raise ValueError(f"{task_name}: Expected columns {expected_cols}, got {list(pred_df.columns)}")
    
    # Validate row count
    if len(pred_df) != len(test_df):
        raise ValueError(f"{task_name}: Prediction count {len(pred_df)} != test count {len(test_df)}")
    
    # Validate no NaNs
    if pred_df.isnull().any().any():
        raise ValueError(f"{task_name}: Contains NaN values")
    ...
    if (pred_df['prediction'] < 0).any():
        raise ValueError(f"{task_name}: Contains negative predictions")
...
def main():
    ...
    try:
        ...
        if task1_source.exists():
            os.rename(str(task1_source), str(task1_target))
            logger.info(f"Moved task1_predictions.csv to {task1_target}")
        
        if task2_source.exists():
            os.rename(str(task2_source), str(task2_target))
