...
# Constants
WEEKEND_START = 5  # Saturday is weekday 5
...
def extract_task2_features(staffing_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "date" in staffing_df.columns:
        ...
        # Weekend indicator
        features_df["is_weekend"] = date_col.dt.weekday >= WEEKEND_START
...
def prepare_task2_features_for_training(
    features_df: pd.DataFrame, staffing_df: pd.DataFrame
...
def clean_features_for_modeling(features_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in numeric_cols:
        if cleaned_df[col].isna().any():
            ...
            fill_val = 0 if col in ["employees_on_duty", "total_task_time_minutes"] else cleaned_df[col].median()
