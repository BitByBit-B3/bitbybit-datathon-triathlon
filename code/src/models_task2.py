...
from baselines import create_task2_baseline
...
class StaffingPatternModel:
    ...
    def fit(self, features_df: pd.DataFrame, target_series: pd.Series):
        ...
        logger.info(
            f"Learned patterns for {len(self.section_patterns)} sections, {len(self.weekday_patterns)} weekdays"
        )
        self.is_fitted = True
        return self
        ...
        for _, row in features_df.iterrows():
            ...
            if "section_id" in row.index and not pd.isna(row["section_id"]):
                ...
                if section_id in self.section_patterns:
                    ...
                    prediction = section_pattern["median"] if section_pattern["count"] >= 3 else section_pattern["mean"]
