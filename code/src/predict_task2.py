...
from features_task2 import clean_features_for_modeling, extract_test_features_task2
from io_safe import read_csv_safe, safe_pickle_load, write_csv_safe
from paths import TASK2_ARTIFACTS_PATH, TASK2_PREDICTIONS_PATH, TASK2_TEST_PATH
...
def main() -> None:
    ...
    try:
        ...
        if not TASK2_TEST_PATH.exists():
            msg = f"Test inputs not found: {TASK2_TEST_PATH}"
        ...
        if not TASK2_ARTIFACTS_PATH.exists():
            msg = f"Model artifacts not found: {TASK2_ARTIFACTS_PATH}"
            raise FileNotFoundError(msg)
        ...
        if "section_id" in test_inputs_df.columns:
            ...
            for section_id, stats in section_stats.iterrows():
                logger.info(
                    f"  Section {section_id}: {stats['count']} predictions, "
...
def validate_predictions() -> bool | None:
    ...
    try:
        ...
        if not TASK2_PREDICTIONS_PATH.exists():
            msg = f"Predictions file not found: {TASK2_PREDICTIONS_PATH}"
            raise FileNotFoundError(msg)
...
def analyze_predictions() -> bool | None:
    ...
    try:
        ...
        section_analysis = (
            analysis_df.groupby("section_id")["prediction"].agg(["count", "mean", "std", "min", "max"]).round(2)
        )
        ...
        for section_id, stats in top_sections.iterrows():
            logger.info(
                f"  Section {section_id}: avg={stats['mean']}, std={stats['std']}, range={stats['min']}-{stats['max']}"
            )
