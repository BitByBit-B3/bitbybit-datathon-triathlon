...
from paths import BOOKINGS_TRAIN_PATH, STAFFING_TRAIN_PATH, TASKS_PATH
from time_utils import (
    calculate_duration_minutes,
    clean_duration_values,
    ...
    for col in numeric_cols:
        if col in df.columns:
            ...
            if len(valid_values) > 0:
                logger.info(
                    f"{col} stats: min={valid_values.min()}, max={valid_values.max()}, mean={valid_values.mean():.1f}"
                )
    ...
    if "employees_on_duty" in df.columns and "total_task_time_minutes" in df.columns:
        ...
        if len(valid_data) > 0:
            ...
            if missing_mask.sum() > 0:
                ...
                logger.info(
                    f"Filled {missing_mask.sum()} missing employees_on_duty values using total_task_time_minutes"
                )
...
def merge_with_tasks(df: pd.DataFrame, tasks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    unmatched_mask = (
        merged_df["task_name"].isna() if "task_name" in tasks_df.columns else merged_df["section_id"].isna()
    )
...
def extract_basic_features(df: pd.DataFrame, feature_cols: list | None = None) -> pd.DataFrame:
    """
