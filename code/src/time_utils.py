...
def parse_datetime_safe(dt_str: str, default_tz: str = "UTC") -> datetime | None:
    """
...
def calculate_duration_minutes(start_series: pd.Series, end_series: pd.Series) -> pd.Series:
    ...
    if valid_mask.sum() > 0:
        ...
        logger.info(
            f"Duration stats: min={valid_durations.min():.1f}, "
...
def parse_time_safe(time_str: str) -> tuple[int, int] | None:
    """
    ...
    try:
        ...
        if ":" in time_str:
            ...
            if len(parts) == HOUR_MINUTE_PARTS:
                hour = int(parts[0])
                minute = int(parts[1])
        ...
        elif len(time_str) == TIME_STR_LENGTH and time_str.isdigit():
            hour = int(time_str[:2])
...
def clean_duration_values(duration_series: pd.Series, min_minutes: float = 0, max_minutes: float = 480) -> pd.Series:
    """
    ...
    if removed_count > 0:
        logger.info(f"Removed {removed_count} invalid durations (outside range {min_minutes}-{max_minutes} minutes)")
